...
> 面向 Coding Agent 的本地优先轨迹观察、会话回放与调试工作台。
...
Agent Trace Workbench 是一个只在本机运行的 Coding Agent 可观测性工具。它可以通过本地 Gateway 记录 Anthropic Messages API 和 OpenAI Responses API 请求，也可以导入 Claude Code、Codex CLI 留在本机的会话历史，然后把不同来源的数据统一成一条可搜索、可回放、可诊断的事件时间线。

它不是云端监控平台，不需要账号，不上传遥测，也不会为了得到“完整轨迹”而伪造模型没有提供的 reasoning（推理内容）。

![Agent Trace Workbench 的 Session Explorer](docs/screenshots/session-explorer.png)

> 截图使用合成数据生成，不包含真实 Agent 对话、API 密钥或本机路径。
...
- **特色界面**：保留动漫风格、内置动漫壁纸、自定义壁纸、透明度、亮度和模糊设置。

## 支持范围

| 层级 | 适配器 | 当前能力 |
|---|---|---|
| Agent | Claude Code | 发现并导入本地 JSONL History；识别主请求与辅助请求；转换消息、thinking、工具和 usage |
| Agent | Codex CLI | 发现并导入本地 rollout JSONL；检测已观察到的格式版本；转换消息、reasoning summary、工具和 usage |
| Protocol | Anthropic Messages API | 解析流式 SSE 与非流式 JSON 响应 |
| Protocol | OpenAI Responses API | 解析流式 SSE 与非流式 JSON 响应 |
| Capture | Local Gateway | 固定路由转发、流式透传、脱敏抓取和事件转换 |
| Capture | Advanced / Legacy MITM | 保留旧的证书代理模式，用于客户端无法设置 Base URL 的场景 |

Codex CLI 支持依据本机实际观察到的 rollout 结构实现，默认发现位置为：
...
Agent / API ─> Gateway ─┤                     │
                         └─> 脱敏后的原始抓取  │
                                               ├─> events.jsonl
本地 Agent History ─> Agent Adapter ──────────┘
                                                     │
                                                     ├─> Session Explorer
                                                     ├─> Replay
                                                     ├─> Diagnostics
